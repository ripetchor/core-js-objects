...
function makeWord(lettersObject) {
  ...
  Object.entries(lettersObject).forEach(([letter, letterIndices]) => {
...
function sellTickets(queue) {
  ...
  while (queue.length) {
    const bill = queue.shift() ?? 0;

    change += bill;

    const nextBill = queue.shift() ?? 0;

    if (change < nextBill / 3) {
      return false;
    }
  }
...
function group(array, keySelector, valueSelector) {
  ...
  array.forEach((value) => {
    ...
    map.get(selectedKey).push(selectedValue);
  });
  return map;
}
